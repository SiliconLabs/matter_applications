...
CHIP_ERROR LightingManager::Init()
{
    ...
    ColorControl::Attributes::CurrentSaturation::Get(1, &mCurrentSat);
    ColorControl::Attributes::CurrentHue::Get(1, &mCurrentHue);
    chip::app::DataModel::Nullable<uint8_t> level;
    LevelControl::Attributes::CurrentLevel::Get(1, level);
...
void LightingManager::SetColorHue(uint8_t hue)
{
  mCurrentHue = hue;
}

void LightingManager::SetColorSaturation(uint8_t sat)
{
  mCurrentSat = sat;
}

uint8_t LightingManager::GetCurrentLevel()
{
  return mCurrentLevel;
}

uint8_t LightingManager::GetCurrentColorHue()
{
  return mCurrentHue;
}

uint8_t LightingManager::GetCurrentColorSaturation()
{
  return mCurrentSat;
}
...
{
    return (mState == kState_OffInitiated
            || mState == kState_OnInitiated
            || mState == kState_MoveToLevelInitiated
            || mState == kState_MoveToHueSatInitiated);
}

bool LightingManager::IsLightOn()
{
    return (mState == kState_OnCompleted
            || mState == kState_MoveToLevelCompleted
            || mState == kState_MoveToHueSatCompleted);
}
...
bool LightingManager::InitiateAction(int32_t aActor, Action_t aAction)
{
    ...
    } else if (mState == kState_OnCompleted
             || mState == kState_MoveToLevelCompleted
             || mState == kState_MoveToHueSatCompleted) {
      if (aAction == OFF_ACTION && mOffEffectArmed == false) {
        action_initiated = true;

        new_state = kState_OffInitiated;
      } else if (aAction == MOVE_TO_LEVEL_ACTION) {
        action_initiated = true;

        new_state = kState_MoveToLevelInitiated;
      } else if (aAction == MOVE_TO_HUE_SAT_ACTION) {
        action_initiated = true;

        new_state = kState_MoveToHueSatInitiated;
      }
    }
...
void LightingManager::ActuatorMovementTimerEventHandler(AppEvent * aEvent)
{
    ...
    else if (light->mState == kState_MoveToLevelInitiated)
    {
        light->mState   = kState_MoveToLevelCompleted;
        actionCompleted = MOVE_TO_LEVEL_ACTION;
    }
    else if (light->mState == kState_MoveToHueSatInitiated)
    {
        light->mState   = kState_MoveToHueSatCompleted;
        actionCompleted = MOVE_TO_HUE_SAT_ACTION;
    }
