...
//< WW2023: Add 4x declarations for alternative images
static const uint8_t OffOff_StateBitMap[] = { LOCK_BULB_00 };
static const uint8_t OnOff_StateBitMap[]  = { LOCK_BULB_10 };
static const uint8_t OffOn_StateBitMap[] = { LOCK_BULB_01 };
static const uint8_t OnOn_StateBitMap[]  = { LOCK_BULB_11 };
//>
...
// WW2023: Add support for dual states
void demoUIDisplayApp(bool on1, bool on2)
{
    //< WW2023: Replace default 2 state image drawing with 4 state version
    /*
    ...
    */
    if(on1)
    {
        if(on2)
        {
            GLIB_drawBitmap(&glibContext, APP_X_POSITION, APP_Y_POSITION, APP_BITMAP_WIDTH, APP_BITMAP_HEIGHT, OnOn_StateBitMap);
        }
        else
        {
            GLIB_drawBitmap(&glibContext, APP_X_POSITION, APP_Y_POSITION, APP_BITMAP_WIDTH, APP_BITMAP_HEIGHT, OnOff_StateBitMap);
        }
    }
    else
    {
        if(on2)
        {
            GLIB_drawBitmap(&glibContext, APP_X_POSITION, APP_Y_POSITION, APP_BITMAP_WIDTH, APP_BITMAP_HEIGHT, OffOn_StateBitMap);
        }
        else
        {
            GLIB_drawBitmap(&glibContext, APP_X_POSITION, APP_Y_POSITION, APP_BITMAP_WIDTH, APP_BITMAP_HEIGHT, OffOff_StateBitMap);
        }
    }
    //>
#if (defined(EFR32MG24) && defined(WF200_WIFI))
